...
mod tests {
    ...
    fn evaluate_ground_goal_term_equality_is_state_independent() {
        ...
        // in.
        assert!(evaluate_ground_goal(&eq("a", "a"), &empty).unwrap());
        assert!(!evaluate_ground_goal(&eq("a", "b"), &empty).unwrap());
        assert!(evaluate_ground_goal(&eq("a", "a"), &facts(&["at(r,a)"])).unwrap());
        ...
        assert!(!relaxed_satisfiable(&eq("a", "b"), &empty).unwrap());
    }
    ...
    fn term_equality_grounds_to_eq_not_an_atom_fact_key() {
        ...
        assert!(action_applicable(&find_action("mark(a)"), &facts(&[])).unwrap());
        assert!(!action_applicable(&find_action("mark(b)"), &facts(&[])).unwrap());
    }
