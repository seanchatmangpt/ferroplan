...
// @ 33e87693, file varisat/src/prop/graph.rs); `lits` takes the clause
// allocator directly instead of a partial context reference.
...
impl Reason {
    ...
    pub fn lits<'a>(&'a self, alloc: &'a ClauseAlloc) -> &'a [Lit] {
        match self {
            Reason::Unit => &[],
            Reason::Binary(lit) => lit,
            // The propagated literal is always kept at position 0
            Reason::Long(cref) => &alloc.clause(*cref).lits()[1..],
        }
    }
...
impl Conflict {
    ...
    pub fn lits<'a>(&'a self, alloc: &'a ClauseAlloc) -> &'a [Lit] {
        match self {
            ...
            Conflict::Long(cref) => alloc.clause(*cref).lits(),
        }
    }
}
...
pub struct ImplNode {
    pub reason: Reason,
    pub level: LitIdx,
    /// Position in trail when assigned, `LitIdx::MAX` is used as sentinel for removed
    /// units.
    pub depth: LitIdx,
}
