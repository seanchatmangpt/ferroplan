...
// @ 33e87693, file varisat/src/prop/watch.rs).
...
pub fn enable_watchlists(ctx: &mut Context) {
    let Context {
        watchlists,
        clause_alloc,
        clause_db,
    ...
    for cref in db::clauses_iter(clause_db, clause_alloc) {
        let lits = clause_alloc.clause(cref).lits();
