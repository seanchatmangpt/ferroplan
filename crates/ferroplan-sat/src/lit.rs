...
// @ 33e87693, file varisat-formula/src/lit.rs).
...
mod tests {
    ...
    #[test]
    fn dimacs_roundtrip() {
        for number in [1isize, -1, 2, -2, 42, -42] {
            assert_eq!(Lit::from_dimacs(number).to_dimacs(), number);
        }
        assert_eq!(Var::from_dimacs(7).to_dimacs(), 7);
    }

    #[test]
    fn negation_and_polarity() {
        let x = Lit::from_dimacs(3);
