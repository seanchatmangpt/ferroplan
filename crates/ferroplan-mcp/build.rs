...
//! every MCP tool group, extracted at compile time from the base Ferroplan
//! ontology and the v26.8.1 probabilistic extension.
...
fn main() {
    ...
    let ontology_paths = [
        ...
        "../../plugins/chatman-ecosystem/ontology/ferroplan-v26.8.1-domain.ttl",
    ];
    let mut ontology = String::new();
    for relative in ontology_paths {
        println!("cargo:rerun-if-changed={relative}");
        let path = Path::new(&manifest_dir).join(relative);
        ontology.push_str(&fs::read_to_string(&path).unwrap_or_else(|error| {
            panic!(
                "ferroplan-mcp/build.rs: failed to read ontology at {}: {error}",
                path.display()
            )
        }));
        ontology.push('\n');
    ...
    let comments = extract_tool_comments(&ontology);
...
fn write_group(
    ...
    generated.push_str("// @generated by ferroplan-mcp/build.rs from Ferroplan ontologies. Do not edit; do not commit (OUT_DIR only).\n");
