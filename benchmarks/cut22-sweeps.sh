#!/usr/bin/env bash
# THE 0.22 CUT SWEEP — all SIXTEEN boards — the thirteen 0.21 boards
# plus the Phase 8 re-entries (propositional, net-benefit, constraints:
# 840 instances leaving the not-re-baselined list) against the final 0.21.0 binary, staged in
# benchmarks/air22/ and promoted by promote-air22.sh. Same driver pattern and
...
#
# Resume-aware: per-board .done markers in benchmarks/air22/. Re-running is
# always safe.
set -u
cd "$(dirname "$0")/.."
mkdir -p benchmarks/air22
...
case "$V" in
  *0.22*) : ;;
  *) echo "REFUSING: binary does not report 0.22 — build the cut candidate first"
     exit 1 ;;
esac
...
run_board() { # name track timeout extra-args...
  ...
  if [ -f "benchmarks/air22/$name.done" ]; then
    echo "SKIP $name (done)"
    return
  fi
  wait_quiet
  ...
  local cond="benchmarks/air22/$name.conditions.json"
  ...
  python3 benchmarks/ipc67.py --track "$track" --timeout "$tmo" \
    ...
    --out "benchmarks/air22/$name.md" >"benchmarks/air22/$name.log" 2>&1
...
print(', '.join(f'{k} {v:.0f}%' for k,v in list(c.items())[:2]) or 'none')" 2>/dev/null || echo '?')
  echo "DONE $name: $(tail -1 "benchmarks/air22/$name.md") $(date '+%H:%M:%S') [idle ${med}% $verdict]"
  if [ "$verdict" = "clean" ]; then
    touch "benchmarks/air22/$name.done"
  else
...
remaining() {  # names of boards not yet banked clean
  ...
  for spec in "${BOARDS[@]}"; do
    n=${spec%% *}
    [ -f "benchmarks/air22/$n.done" ] || out="$out $n"
  done
  echo "$out"
}
...
echo "0.22 CUT SWEEP ALL DONE $(date '+%Y-%m-%d %H:%M:%S') (passes: $((pass - 1)))"
echo "next: benchmarks/promote-air22.sh"
