#!/usr/bin/env bash
# Promote the 0.28 boards into benchmarks/, then regenerate the standings.
# ONE staging dir: benchmarks/air28/ — the 32-board `cut28` set, swept by the
# crucible (docs/roadmap-0.28.md, "The cut"). The 0.27 promoted raws are the
# referee; the regression re-check (every instance 0.27 solved that 0.28 did
# not, re-run through the crucible as a subset: `crucible compare --lost`,
# then `sweep --rows`) runs BEFORE this script.
#
# Run ONLY after `benchmarks/cut28-sweep.log` says SWEEP COMPLETE — crucible
...
if [ "$missing" -ne 0 ] || ! grep -q 'SWEEP COMPLETE' benchmarks/cut28-sweep.log; then
...
for name in sys.argv[1:]:
    ...
    if reg is None:
        ...
        continue
    path = os.path.join("benchmarks/air28", name + ".jsonl")
    with open(path) as f:
...
echo "  python3 scripts/standings-snapshot.py --version 0.28.0 --measured-at YYYY-MM-DD"
echo "  CHANGELOG.md [0.28.0] + README 'What's new in 0.28.0' + Status line"
