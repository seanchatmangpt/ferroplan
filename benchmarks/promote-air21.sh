#!/usr/bin/env bash
# Promote the 0.21 cut boards from benchmarks/air21/ into benchmarks/, then
# regenerate the standings. Run ONLY after cut21-sweeps.sh reports ALL DONE —
# a half-promoted set would mix a 0.21 board with a 0.20 one under the same
# name, exactly what the promotion gate exists to prevent (promote-air.sh).
set -eu
cd "$(dirname "$0")/.."
AIR=benchmarks/air21

need=(ipc2026-opt ipc2023-agile ipc2014-tempo ipc2018-sat ipc2014-sat
