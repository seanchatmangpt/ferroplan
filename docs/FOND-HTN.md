...
| Objects typed with an undeclared type (reference parser: silently dropped from the parsed model — harvest evidence, ticket fond-htn-02: 14 problem references to the undeclared `FaultyPort` became 0 objects, parser *and* grounder exit 0, no diagnostic) | `ValidationError::UndefinedType` at validation, before grounding — a loud typed refusal (`crates/ferroplan-hddl/src/validate.rs`) | Parity divergence: ferroplan refuses what the reference silently drops; a domain the reference "solves" by dropping objects is rejected here |
